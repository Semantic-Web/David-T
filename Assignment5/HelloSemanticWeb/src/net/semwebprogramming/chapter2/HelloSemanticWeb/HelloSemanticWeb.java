...
public class HelloSemanticWeb {
	...
	private void applySelfRule(Model model){
		//Hello to just Me - Filtering only my name
		runQuery(" select DISTINCT ?name where{ people:me foaf:name ?name. FILTER(?name = \"David Terrell\")}", model);  //add the query string

		String rules = "[applySelf: (?person <http://org.semwebprogramming/chapter2/people#Individual_5> ?name) -> (?person <http://org.semwebprogramming/chapter2/people#me> ?name)]";
